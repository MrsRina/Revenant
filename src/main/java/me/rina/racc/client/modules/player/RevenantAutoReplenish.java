...
public class RevenantAutoReplenish extends RevenantModule
{
    public RevenantAutoReplenish() {
        super("AutoReplenish", "AutoReplenish", "AutoReplenish", Category.PLAYER);
    }

    private RevenantSetting threshold = newSetting(new String[] {"Threshold", "AutoReplenishThreshold", "Threshold"}, 1, 0, 63);
    private RevenantSetting tickDelay = newSetting(new String[] {"TickDelay", "AutoReplenishTickDelay", "TickDelay"}, 1, 0, 10);
    ...
    public void onUpdate() {
        ...
        if (delayStep < tickDelay.getInteger()) {
            delayStep++;
            return;
        } else {
            delayStep = 0;
    ...
    private Pair<Integer, Integer> findReplenishableHotbarSlot() {
        ...
        for (Map.Entry<Integer, ItemStack> hotbarSlot : getHotbar().entrySet()) {
            ...
            if (stack.stackSize > threshold.getInteger()) {
                continue;
            }
